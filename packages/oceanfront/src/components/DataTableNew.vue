  ...
  >
    ...
    <template :key="rowidx" v-for="(row, rowidx) of rows">
      <of-table-row
        ...
        :idx="rowidx"
        @update:row="(data) => updateRow(rowidx, data)"
      ></of-table-row>
    </template>
  ...
  setup(props, ctx) {
    ...
    const dragEvents = {
      ...
      setCoords: (data: any) => {
        currentCoords.value = data.coords
        nestDepth.value = data.depth
        appendLast.value = data.appendLast
    ...
    const switchItems = (itemIndexes: number[], targetIndexes: number[]) => {
      ...
      if (itemIndexes.length && targetIndexes.length) {
        if (samePosition(itemIndexes, targetIndexes)) return
    ...
    const updateRow = (index: any, row: any) => {
